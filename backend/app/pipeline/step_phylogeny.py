...
MAX_PHYLOGENY_ASVS = 500  # cap to keep pdist fast (~125k pairs max)
...
class PhylogenyRunner:
    ...
    def run(self) -> Tuple[Path, Path]:
        ...
        # Cap to MAX_PHYLOGENY_ASVS so pdist stays fast
        if n > MAX_PHYLOGENY_ASVS:
            logger.warning(
                "Phylogeny: %d ASVs exceeds MAX_PHYLOGENY_ASVS=%d; "
                "sub-sampling representatives.", n, MAX_PHYLOGENY_ASVS
            )
            rng = np.random.default_rng(42)
            chosen = rng.choice(n, MAX_PHYLOGENY_ASVS, replace=False)
            chosen.sort()
            X = X[chosen]
            ids_present = [ids_present[i] for i in chosen]
            n = MAX_PHYLOGENY_ASVS
